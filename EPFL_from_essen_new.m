                ...
                for m=methods_to_test
                   ...
                   [P1,P2,P3] = tft_from_3_essmat(data.E_est{j,i},...
                    ...
                    if repr_err(it,m,2) < 1
                        ...
                        gttt = T_from_P({P1,P2,P3});
                        ba_result = T_from_P({R_t_ref(1:3,:),R_t_ref(4:6,:), R_t_ref(7:9,:)});
                        real_gttt = T_from_P({ccams(:,:,i),ccams(:,:,j),ccams(:,:,k)});                       
                        sFlip = sign(sum(sign(real_gttt .* gttt),'all'));
                        if sFlip == -1
                            sFlip_count = sFlip_count + 1;
                        end    
                        cTest{im2,im3,im1} = sFlip * gttt;


                        gttt = T_from_P({P1,P3,P2});
                        ba_result = T_from_P({R_t_ref(1:3,:),R_t_ref(7:9,:), R_t_ref(4:6,:)});
                        real_gttt = T_from_P({ccams(:,:,i),ccams(:,:,k),ccams(:,:,j)});                       
                        sFlip = sign(sum(sign(real_gttt .* gttt),'all'));
                        if sFlip == -1
                            sFlip_count = sFlip_count + 1;
                        end
                        cTest{im3,im2,im1} = sFlip * gttt;


                        gttt = T_from_P({P2,P1,P3});
                        ba_result = T_from_P({R_t_ref(4:6,:),R_t_ref(1:3,:), R_t_ref(7:9,:)});
                        real_gttt = T_from_P({ccams(:,:,j),ccams(:,:,i),ccams(:,:,k)});  
                        sFlip = sign(sum(sign(real_gttt .* gttt),'all'));
                        if sFlip == -1
                            sFlip_count = sFlip_count + 1;
                        end
                        cTest{im1,im3,im2} = sFlip * gttt;


                        gttt = T_from_P({P2,P3,P1});
                        ba_result = T_from_P({R_t_ref(4:6,:),R_t_ref(7:9,:), R_t_ref(1:3,:)});
                        real_gttt = T_from_P({ccams(:,:,j),ccams(:,:,k),ccams(:,:,i)});  
                        sFlip = sign(sum(sign(real_gttt .* gttt),'all'));
                        if sFlip == -1
                            sFlip_count = sFlip_count + 1;
                        end
                        cTest{im3,im1,im2} = sFlip * gttt;


                        gttt = T_from_P({P3,P1,P2});
                        ba_result = T_from_P({R_t_ref(7:9,:),R_t_ref(1:3,:), R_t_ref(4:6,:)});
                        real_gttt = T_from_P({ccams(:,:,k),ccams(:,:,i),ccams(:,:,j)});  
                        sFlip = sign(sum(sign(real_gttt .* gttt),'all'));
                        if sFlip == -1
                            sFlip_count = sFlip_count + 1;
                        end
                        cTest{im1,im2,im3} = sFlip * gttt;

                        gttt = T_from_P({P3,P2,P1});
                        ba_result = T_from_P({R_t_ref(7:9,:),R_t_ref(4:6,:), R_t_ref(1:3,:)});
                        real_gttt = T_from_P({ccams(:,:,k),ccams(:,:,j),ccams(:,:,i)});  
                        sFlip = sign(sum(sign(real_gttt .* gttt),'all'));
                        if sFlip == -1
                            sFlip_count = sFlip_count + 1;
                        end
                        cTest{im2,im1,im3} = sFlip * gttt;
