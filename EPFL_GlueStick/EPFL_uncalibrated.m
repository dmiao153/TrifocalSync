        ...
        for k = 1:n
            ...
            %%%-==================================================-%%%
                ...
                for m=methods_to_test
                    ...
                    t0 = cputime;
                    [T, distances2, Corresp_inliers, R_t_2, R_t_3, inlier_indices] = STE_local_optimization(T, Corresp', distances, 0.4, CalM);
                    t=cputime-t0;
...
%                     iter(it,m,1)=nit; time(it,m,1)=t;
                    ...
                    ref_R_t = [CalM(1:3,:)*R_t_ref(1:3,:);CalM(4:6,:)*R_t_ref(4:6,:); CalM(7:9,:)*R_t_ref(7:9,:)];
                    ...
                    if repr_err(it,m,2) < 1
                        cTest{im2,im3,im1} = T_from_P({ref_R_t(1:3,:),ref_R_t(4:6,:), ref_R_t(7:9,:)});
                        cTest{im3,im2,im1} = T_from_P({ref_R_t(1:3,:),ref_R_t(7:9,:), ref_R_t(4:6,:)});
                        cTest{im1,im3,im2} = T_from_P({ref_R_t(4:6,:),ref_R_t(1:3,:), ref_R_t(7:9,:)});
                        cTest{im3,im1,im2} = T_from_P({ref_R_t(4:6,:),ref_R_t(7:9,:), ref_R_t(1:3,:)});
                        cTest{im1,im2,im3} = T_from_P({ref_R_t(7:9,:),ref_R_t(1:3,:), ref_R_t(4:6,:)});
                        cTest{im2,im1,im3} = T_from_P({ref_R_t(7:9,:),ref_R_t(4:6,:), ref_R_t(1:3,:)});
                    else
