...
%                 two_view_corresp{i,j} = get_distinct_elements(two_view_corresp{i,j}, corresp_by_triplet{i,j,k}(:,[1,2,3,4]));
%                 two_view_corresp{i,k} = get_distinct_elements(two_view_corresp{i,k}, corresp_by_triplet{i,j,k}(:,[1,2,5,6]));
%                 two_view_corresp{j,k} = get_distinct_elements(two_view_corresp{j,k}, corresp_by_triplet{i,j,k}(:,[3,4,5,6]));
%             end
%         end
%     end
% end
% 
% save("EPFL_code/"+dataset+"_two_view_corresp.mat", "two_view_corresp")
...
for i = 1:n-1
    for j = i+1:n
        if size(two_view_corresp{i,j},1) > 16
            % [essenmats{i,j}, inliersIndex] = estimateEssentialMatrix(two_view_corresp{i,j}(:,[1,2]), two_view_corresp{i,j}(:,[3,4]), cam_intrinsics{i}, cam_intrinsics{j}, 'MaxNumTrials', 1000);
            % inliers_idx{i,j} = inliersIndex;
            % essenmats{j,i} = essenmats{i,j}';
            ...
            % cur_relativePose = estrelpose(cur_essen', cam_intrinsics{j}, cam_intrinsics{i}, ...
            %     two_view_corresp{i,j}(:,3:4), two_view_corresp{i,j}(:,1:2));

            % ba_sample = randsample(1:size(two_view_corresp{i,j},1), min(50, size(two_view_corresp{i,j},1)));
            % 
            % [R_t_ref,~,nit,repr_errBA]=BundleAdjustment([Ks(:,:,i); Ks(:,:,j)],...
            %     [eye(3,4); [cur_relativePose.R, -cur_relativePose.Translation']], transpose(two_view_corresp{i,j}(:, [1,2,3,4])));
            % % repr_err=ReprError({eye(3,4), [cur_relativePose.R, -cur_relativePose.Translation']}, ...
            % %     transpose(two_view_corresp{i,j}(inliersIndex,:)))
            % % 
            % % [errR_mean, errR_median, errT_mean, errT_median] = comparison_for_calibrated_cameras(...
            % %     {eye(3,4), [cur_relativePose.R, -cur_relativePose.Translation']},{ccams(:,:,i), ccams(:,:,j)})
            % % 
            % % [errR_mean, errR_median, errT_mean, errT_median] = comparison_for_calibrated_cameras(...
            % %     {R_t_ref(1:3,:), R_t_ref(4:6,:)},{ccams(:,:,i), ccams(:,:,j)})
            % 
            % repr_errBA
            % repr_err=ReprError({Ks(:,:,i) * eye(3,4), Ks(:,:,j) * [cur_relativePose.R, -cur_relativePose.Translation']}, ...
            %     transpose(two_view_corresp{i,j}(inliersIndex,:)))
            % if repr_err < 1
                % new_e = vgg_F_from_P(R_t_ref(1:3,:), R_t_ref(4:6,:));
                % essenmats{i,j} = new_e / norm(new_e);
            essenmats{i,j} = cur_essen;
            essenmats{j,i} = essenmats{i,j}';
            % end
        end
    end
...
[rotation_metrics, angle_metrics] = Compare_estimated_gt_relative_rotations(data.G_gt, Hmat);
subplot(1,2,1)
...
save("EPFL_code/"+dataset+"_two_view_data.mat", "data");
...
for i = 1:n-1
    for j = i+1:n
        if size(essenmats{i,j},1)>0
            relativePose = estrelpose(essenmats{i,j}, cam_intrinsics{i}, cam_intrinsics{j}, two_view_corresp{i,j}(:,1:2), two_view_corresp{i,j}(:,3:4));
...
for i = 1:n-1
    for j = i+1:n
        if size(ematcell{i,j},1)>0 
            E_est(3*(i-1)+1:3*i, 3*(j-1)+1:3*j) = ematcell{i,j};
...
for i= 1:n-1
    for j = i+1:n
        ...
        gtessenmats{i,j} = cur_ess / norm(cur_ess);
    end
end
