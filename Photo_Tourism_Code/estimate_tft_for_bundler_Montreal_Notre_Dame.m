...
first_filtering = datacounts > (0.30 * n^2);
...
% final_good_index = [];
% for i = 1:n
%     if sum(data_count(:,:,i),'all') >= (0.1 * n^2)
%         final_good_index = [final_good_index, i];
%     end
% end
% 
% 
% fprintf("The size of the second filtering is %d\n", length(final_good_index));
% tft_estimated_cams = tft_estimated_cams(final_good_index, final_good_index, final_good_index);
% cameras_gt = cameras_gt(final_good_index);
% E_est = E_est(final_good_index, final_good_index);
% n = length(final_good_index);
% data_count = data_count(final_good_index, final_good_index, final_good_index);
...
diary Photo_Tourism_cleaning/Montreal_Notre_Dame/output.log
